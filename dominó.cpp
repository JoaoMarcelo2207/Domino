...
//Verifica se da pra encaixar a pedra na mesa naquela posicao
int encaixa(tp_listad *mesa, tp_pedra jogada, char escolha){
	tp_no *atu, *atu2;
	atu = mesa->ini;
	atu2 = mesa->fim;   //Lado1 = esquerda
						//Lado2 = direita
	//printf("peca recebida: %d/%d\n", jogada.lado1, jogada.lado2);			
	
	if(escolha == 'D' || escolha =='d'){   //encaixa na direita
		// if(atu1->info.lado1 == atu2->info.lado2){
		//printf("encaixa na direita\n");
		
		if(atu2->info.lado2 == jogada.lado1){
			//printf("caso 1: encaixa sem inverter\n");
			insere_listad_no_fim(mesa, jogada);
			return 1;
		
		}else if(atu2->info.lado2 == jogada.lado2){
			
				//printf("caso 2: encaixa inverter\n");
				inverte_peca(&jogada);
				//printf("peca invertida: %d/%d\n", jogada.lado1, jogada.lado2);
				insere_listad_no_fim(mesa, jogada);
				//printf("inseriu\n");
				return 1;
...
int checa_encaixe(tp_listad *mesa, tp_pedra pedra) {
	...
	//		printf("Essa pedra encaixa na direita.\n");
			return 1;
		}
	//	printf("Essa pedra nao encaixa na mesa.\n");
		return 0;
	}else{
...
int checa_joga(tp_listad *mesa, tp_listad *mao ){
	...
	//		printf("Achei uma peca que encaixa!!!\n");
			return 1;
		}	
			
		cont++;
	}
	//printf("Nao tem nenhuma peca encaixa :(\n");
	return 0;
}
